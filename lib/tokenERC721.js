...
class TokenERC721Contract extends Contract {
    constructor() {
        ...
        this.nftPrefix = "nft";
        this.approvalPrefix = "approval";

        // Define key names for options
        this.nameKey = "name";
        this.symbolKey = "symbol";
        this.centralBanKer = "ManagerMSP";
    }
    ...
    async TransferFrom(ctx, from, to, tokenId) {
        ...
        const operatorApproval = await this.IsApprovedForAll(
            ctx,
            owner,
            sender
        );
        ...
        const balanceKeyFrom = ctx.stub.createCompositeKey(this.balancePrefix, [
            from,
            tokenId,
        ]);
        ...
        const balanceKeyTo = ctx.stub.createCompositeKey(this.balancePrefix, [
            to,
            tokenId,
        ]);
    ...
    async SetApprovalForAll(ctx, operator, approved) {
        ...
        const approvalKey = ctx.stub.createCompositeKey(this.approvalPrefix, [
            sender,
            operator,
        ]);
    ...
    async Initialize(ctx, name, symbol) {
        ...
        const nameBytes = await ctx.stub.getState(this.nameKey);
     ...
     * @param {Object} tokenParams containing metadata of the minted non-fungible token
    ...
    async Burn(ctx, tokenId) {
        ...
        if (nft.owner !== owner) {
            throw new Error(
                `Non-fungible token ${tokenId} is not owned by ${owner}`
            );
        }
    ...
    async _readNFT(ctx, tokenId) {
        ...
        if (!nftBytes || nftBytes.length === 0) {
            throw new Error(
                `The tokenId ${tokenId} is invalid. It does not exist`
            );
        }
     ...
     * @param {Context} ctx the transaction context
     * @param {string} tokenId The identifier for a non-fungible token
     * @returns {String} Returns the token JSON
     */
